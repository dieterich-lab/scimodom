...
from scimodom.api.comparison import (
    dataset_comparison_api,
    IntersectResponse,
    ClosestResponse,
    SubtractResponse,
)
...
def test_client():
    app = Flask(__name__)
    app.register_blueprint(dataset_comparison_api, url_prefix="")
...
def mock_services(mocker):
    ...
    mocker.patch(
        "scimodom.api.comparison.get_data_service", return_value=MockDataService()
    )
    mocker.patch(
        "scimodom.api.comparison.get_validator_service",
        return_value=MockValidatorService(),
    )
    mocker.patch(
        "scimodom.api.comparison.get_bedtools_service",
        return_value=MockBedtoolsService(),
    )
    mocker.patch("scimodom.api.comparison.Bed6Importer", return_value=MockBed6Importer)
    mocker.patch("scimodom.api.comparison.EufImporter", return_value=MockEufImporter)
