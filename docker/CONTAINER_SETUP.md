...
Use the password found in _secrets/mariadb-root_. In the DEV setup use instead:

```bash
podman-compose -f docker-compose-db-only.yml  exec scimodom_db_dev mariadb -u root -p scimodom
```
