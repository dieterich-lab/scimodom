...
class Modomics(Base):
    ...
    modifications: Mapped[List["Modification"]] = relationship(back_populates="inst_modomics")
...
class DetectionMethod(Base):
    ...
    technologies: Mapped[List["DetectionTechnology"]] = relationship(back_populates="inst_method")
...
class Taxonomy(Base):
    ...
    kingdom: Mapped[str] = mapped_column(String(32), nullable=False)
    phylum: Mapped[str] = mapped_column(String(32), nullable=False)
...
class Selection(Base):
    ...
    technologies: Mapped["DetectionTechnology"] = relationship(back_populates="selections")
    organisms: Mapped["Organism"] = relationship(back_populates="selections")
    
    associations: Mapped[List["Association"]] = relationship(back_populates="selections")
...
class Dataset(Base):
    ...
    modification_type: Mapped[str] = mapped_column(String(32), nullable=False) # DNA or RNA - in principle the latter only...
    taxa_id: Mapped[int] = mapped_column(ForeignKey("ncbi_taxa.id")) # redundant - Selection or allow for "double checking"
    ...
    annotation_version: Mapped[str] = mapped_column(String(128), nullable=False) # VARCHAR or INTEGER - can we fix this at upload ?
