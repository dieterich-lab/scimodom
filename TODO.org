...
- [ ] Form validation and warning/message styling for login/sign up/reset components (re-usable style binding with sensible defaults).
  Check vee-validate usage.i Inline message (PV)
- [ ] Large number don't show nicely in the Search paginator.
- [ ] We have not updated the preset for Lara. Maybe just remove them?
